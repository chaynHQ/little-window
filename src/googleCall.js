...
const getResource = (countryObj, lang) => {
  ...
  return request(url).then((body) => JSON.parse(body).values.reduce((acc, cur) => [...acc, { text: cur[0], href: cur[1] }], []));
};

const googleCall = (selectedCountries, lang) => selectedCountries.map((countryObj) => getResource(countryObj, lang));
